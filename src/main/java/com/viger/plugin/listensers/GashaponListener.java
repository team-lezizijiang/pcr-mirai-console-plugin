    ...
    public void onMessage(@NotNull GroupMessageEvent event) {
        ...
        if (messageInString.contains("#十连")) {
            if (Gashapon.isCool(sender.getId())) {
                event.getSubject().sendMessage(new At(sender.getId()).plus(plugin.gashapon.gacha(10)));
                Gashapon.refreshCoolDown(sender.getId());
            } else {
                //发送冷却提示消息
                event.getSubject().sendMessage(new At(sender.getId()).plus("抽卡抽的那么快，人家会受不了的"));
            }
        } else if (messageInString.contains("#井")) {
            if (Gashapon.isCool(sender.getId())) {
                event.getSubject().sendMessage(new At(sender.getId()).plus(plugin.gashapon.gacha(300)));
                Gashapon.refreshCoolDown(sender.getId());
            } else {
                //发送冷却提示消息
                event.getSubject().sendMessage(new At(sender.getId()).plus("抽卡抽的那么快，人家会受不了的"));
            ...
            if (Gashapon.isCool(sender.getId())) {
                ...
                try {
                    ...
                    event.getSubject().sendMessage(new At(sender.getId()).plus(plugin.gashapon.gacha(q)));
                    Gashapon.refreshCoolDown(sender.getId());
